...
YOLO_MODEL_PATH = "/home/yousof/Fine-Tuning-SAM2-UNet-for-Breast-Image-Segmentation-Without-Pectoral-Muscle-and-Artifacts/runs/detect/train3/weights/best.pt"
...
def run_inference_from_array(image_array, checkpoint_path=CHECKPOINT_PATH, input_size=INPUT_SIZE, num_classes=NUM_CLASSES):
    """
    Run inference on an image array (numpy array or PIL Image)
    
    Args:
        image_array: numpy array (H, W, 3) RGB or PIL Image
        checkpoint_path: Path to model checkpoint
        input_size: Input size for the model
        num_classes: Number of segmentation classes
    ...
    # Run inference
    with torch.no_grad():
        res, _, _ = model(img_tensor)
        res = F.interpolate(res, size=(original_size[1], original_size[0]), 
                          mode='bilinear', align_corners=False)
        pred_class = torch.argmax(res, dim=1).squeeze().cpu().numpy()
        binary_mask = np.where(pred_class > 0, 255, 0).astype(np.uint8)
...
def load_yolo_model(model_path=YOLO_MODEL_PATH):
    """Load YOLO model for vertebra detection"""
    return YOLO(model_path)

def process_image(image_rgb, yolo_model=None):
    """
    Process an image through the full pipeline.
    
    Args:
        image_rgb: numpy array (H, W, 3) in RGB format
        yolo_model: Pre-loaded YOLO model (optional, will load if None)
    
    Returns:
        dict with keys:
            - 'images': Dict with 4 visualization combinations
            - 'slip_percentages': Dict of vertebra pairs and their slip percentages
            - 'vertebra_data': Transformed vertebra data
    """
    # Run segmentation inference
    binary_mask = run_inference_from_array(image_rgb)
    ...
    for box in results[0].boxes:
        ...
        if cls_id in VERTEBRA_CLASSES:
            ...
            if corners is not None:                
                ...
                corners_absolute[:, 0] += box_coords[0]
                corners_absolute[:, 1] += box_coords[1]
                vertebra_data[vertebra_name]['corners_absolute'] = corners_absolute

    # Image Alignment and transform data
    aligned_img, vertebra_data_transformed = align_spine_and_get_transform(image, vertebra_data)
    ...
    slope = calculate_slope((l5_bottom_1, l5_bottom_2), (s1_bottom_1, s1_bottom_2), None)
    new_pairs = transform_pairs(Pairs, slope)

    # Calculate slip distances
    slip_percentages = {}
    intersection_points = {}
    for key, value in new_pairs.items():
        result = calculate_slip_distance(value)
        slip_percentages[key] = result['slip_percentage']
        intersection_points[key] = result['intersection_point']

    # Generate all 4 visualization combinations
    images = {}
    for vis_lines in [True, False]:
        for vis_corners in [True, False]:
            ...
            if vis_corners:
                for key, value in vertebra_data_transformed.items():
                    for corner in value['corners_absolute']:
                        cv2.circle(aligned_img_to_vis, tuple(corner), 5, (255, 0, 0), -1)

            # Draw intersection points and pairs on all images
            for key in intersection_points:
                cv2.circle(aligned_img_to_vis, tuple(map(int, intersection_points[key])), 3, (255, 255, 0), -1)

            for (v1, v2), corner_pairs in Pairs.items():
                for (corner1, corner2) in corner_pairs:
                    cv2.line(aligned_img_to_vis, corner1, corner2, (255, 0, 255), 2)

            # Convert to RGB and store
            aligned_img_rgb = cv2.cvtColor(aligned_img_to_vis, cv2.COLOR_BGR2RGB)
            images[(vis_lines, vis_corners)] = aligned_img_rgb

    return {
        'images': images,
        'slip_percentages': slip_percentages,
        'vertebra_data': vertebra_data_transformed
    }
...
def main():
    ...
    for key, value in new_pairs.items(): 
        ...
        cv2.circle(aligned_img_to_vis, tuple(map(int, result['intersection_point'])), 3, (255, 255, 0), -1)
